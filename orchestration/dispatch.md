...
目标：让 Claude、Codex、DSH、DevIn 四个 Agent，在**各自的测试项目目录**（`l4d/claude`、`l4d/codex`、`l4d/dsh`、`l4d/devin`）里，读取**同一份**任务书 `TASK.md`，产出到各自命名空间，最后统一按 `verify.sh` 打分。
...
| `run_harness.sh` | 驱动 Claude + Codex + Devin 三条外部 CLI 的编排脚本（并行 + 日志） |
| `verify_template.sh` | 每个 Agent 交付物 `verify.sh` 的模板 |

## 派遣方式

### 方式一：run_harness.sh（推荐）
驱动三条外部 CLI（Claude Code、Codex、Devin）并行跑同一任务：
...
- 脚本会实例化 `${AGENT}` 为 `CLAUDE` / `CODEX` / `DSH` / `DEVIN`，生成四个逐字节一致的 prompt（`runs/<ts>/prompts/{CLAUDE,CODEX,DSH,DEVIN}.md`）。
...
- 结果/日志写入 `_harness-backup/runs/<timestamp>/`（`{CLAUDE,CODEX,DEVIN}.result*` + `.exit`）。
- **DSH 路**：由编排者（当前 harness）用 `subagent` 驱动，prompt 用 `runs/<ts>/prompts/DSH.md`（脚本不负责这一路）。

### 方式二：手动多连（回归复核）
1. 依次进入 `l4d/claude`、`l4d/codex`、`l4d/dsh`、`l4d/devin`。
2. 对每个项目，向对应 CLI（`claude` / `codex` / `devin` / DSH 会话）发送 `dispatch_prompt.md` 实例化后的同一句提示。
3. 记录每个项目的产出 commit、verify.sh 结果。

## 验收打分
- 自动项：`for p in claude codex dsh devin; do (cd ../$p && bash ${AGENT}_out/verify.sh); done`
...
- 派发前，确认 `_harness-backup` 内容已同步进四个项目，`TASK.md` 除 agent 标记外逐字节一致。
